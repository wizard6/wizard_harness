...
> 本地副本可放 `temp/Ombre-Brain`（不入库）。

## 1. 它是什么

给 **模型自己** 用的长期情绪记忆系统：Markdown 桶 + Russell 效价/唤醒度坐标 + 改进艾宾浩斯衰减 + 混合检索（关键词 / BM25 / 向量）+ 器官化工具（`breath` / `hold` / `grow` / `trace` …）。
...
**硬规则**：遗忘 = 淡入 `archive/`，不是物理删除；读 ≠ 强化，强化需显式 `trace(reinforce=True)`。
