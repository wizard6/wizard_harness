...
const agentPlugin: Plugin = {
  manifest: {
    ...
    version: '0.1.0',
    name: 'Agent',
    description: '每个 live agent 一个 scope，绑定一条 session。不管模型/工具循环。',
    provides: ['agent'],
    config: {},
    tier: 'standard',
  },
  ...
  ui: {
    ...
    content: [
      ...
      '<h1>Agent</h1>',
      '<p class="desc">ctx.agent.spawn / get / list / stop。每个实例一个 createScope，绑定一条 session。编排 llm+tools 是后续 agent-loop，不在本插件。</p>',
