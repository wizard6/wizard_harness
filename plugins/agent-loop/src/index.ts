...
const agentLoopPlugin: Plugin = {
  manifest: {
    ...
    version: '0.1.0',
    name: 'Agent 循环',
    description: '编排 llm.complete + tools.call。System Prompt 走独立插件，本插件只 apply。',
    provides: ['agentLoop'],
    config: { maxSteps: 8 },
    tier: 'standard',
  },
  ...
  ui: {
    ...
    content: [
      ...
      '<h1>Agent 循环</h1>',
      '<p class="desc">ctx.agentLoop.run({ agentId?, prompt?, maxSteps? })。经 agent 的 scoped ctx 调 llm / tools。System Prompt 用 ctx.systemPrompt.set/apply，循环开始时 apply。</p>',
      ...
      '<div class="row"><span class="k">可选</span><span class="v">systemPrompt.apply</span></div>',
